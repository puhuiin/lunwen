...
score_path = os.path.join(OUT, f'六维能力复合得分_含L1_{TODAY}.csv')
if not os.path.exists(score_path):
    score_path = find_data(f'六维能力复合得分_含L1_{TODAY}.csv', '分析面板')
score = pd.read_csv(score_path)

panel = pd.read_csv(find_data(f'分析面板_v3_{TODAY}.csv', '分析面板'), parse_dates=['report_date'])
...
tim = pd.read_csv(find_data(f'择时系数_季度HM_{TODAY}.csv', '分析面板')).set_index('fund_code')[['timing']]
