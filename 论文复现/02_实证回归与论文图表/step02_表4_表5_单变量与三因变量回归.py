...
输出 output/单变量全表_三因变量_2026-09-02.json / .csv
"""
import json, os
...
panel = pd.read_csv(find_data('分析面板_v3_2026-08-26.csv', '分析面板'), parse_dates=['report_date'])
v2 = pd.read_csv(find_data('指标面板_v2_2026-08-26.csv', '分析面板'), parse_dates=['report_date'])
...
newp = pd.read_csv(find_data('L2新增候选_面板_2026-09-02.csv', '分析面板'), parse_dates=['report_date'])
rcm = pd.read_csv(find_data('认知指标_rc追涨杀跌_2026-08-25.csv', '分析面板'), parse_dates=['report_date'])
panel = panel.merge(rcm[['fund_code', 'report_date', 'rc_mom']],
                    on=['fund_code', 'report_date'], how='left')
panel = panel.merge(newp[['fund_code', 'report_date', 'anchor_high', 'cgo',
                          'house_money', 'bhm_shm']],
                    on=['fund_code', 'report_date'], how='left')
tim = pd.read_csv(find_data('择时系数_季度HM_2026-08-26.csv', '分析面板')).set_index('fund_code')[['timing']]

# ---------- 因变量三口径 ----------
idx_path = find_data('市场指数历史数据.csv', '基金基础信息')
...
info = pd.read_csv(find_data('基金详细信息_最终版.csv', 'L1_背景特征层'))
mgr = pd.read_csv(find_data('基金经理信息_最终版.csv', 'L1_背景特征层'))
...
with open(os.path.join(OUT, '单变量全表_三因变量_2026-09-02.json'), 'w', encoding='utf-8') as f:
    json.dump(res, f, ensure_ascii=False, indent=1)
fm.reset_index().to_csv(os.path.join(OUT, '单变量全表_基金层数据_2026-09-02.csv'),
                        index=False, encoding='utf-8-sig')
print('\n已保存 output/单变量全表_三因变量_2026-09-02.json')
...
for y, lab in DVS:
    v = res['联立'][y]
    print('\n--- DV = %s（N=%d，R2=%.4f，%d 个指标）---' % (lab, v['n'], v['r2'], v['变量数']))
...
with open(os.path.join(OUT, '单变量全表_三因变量_2026-09-02.json'), 'w', encoding='utf-8') as f:
    json.dump(res, f, ensure_ascii=False, indent=1)
print('\n已更新 output/单变量全表_三因变量_2026-09-02.json（含联立与显著性汇总）')
