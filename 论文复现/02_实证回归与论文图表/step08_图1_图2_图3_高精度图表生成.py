...
# 跨平台中文字体自适应（Mac / Windows / Linux 完美兼容）
plt.rcParams['font.sans-serif'] = [
    'PingFang SC', 'Heiti SC', 'STHeiti', 'Hiragino Sans GB',  # macOS 默认中文字体
    'Microsoft YaHei', 'SimHei', 'SimSun',                      # Windows 默认中文字体
    'WenQuanYi Micro Hei', 'Noto Sans CJK SC',                 # Linux 默认中文字体
    'sans-serif'
]
...
def rj(name):
    ...
    if not os.path.exists(target):
        raw_target = os.path.join(BASE, '原始数据', '分析面板', name)
        if os.path.exists(raw_target):
            target = raw_target
        elif '六维复合定稿验证' in name:
            from step03_表6_六维能力主回归 import run_composite_l1
            run_composite_l1(OUT)
            target = os.path.join(OUT, name)
        elif '画像_群体与典型' in name:
            import subprocess
            subprocess.run([sys.executable, os.path.join(os.path.dirname(__file__), 'step05_表9_表10_表11_群体与典型画像.py')], cwd=BASE)
            target = os.path.join(OUT, name)
...
fig, ax = plt.subplots(figsize=(7.4, 4.5), num="论文图1：综合能力五分组单调性")
...
fig, ax = plt.subplots(figsize=(7.0, 5.8), num="论文图2：六维能力相关热力图")
fig.patch.set_facecolor('white')
...
fig = plt.figure(figsize=(13.4, 11.8), num="论文图3：典型投资经理六维能力雷达图")
fig.patch.set_facecolor('white')
...
print('saved:', p3)
if os.environ.get('SHOW_FIGS', '1') != '0' and '--no-popup' not in sys.argv:
    print('\n[交互弹窗] 正在弹出论文图1、图2、图3展示窗口（在弹出的图形窗口中可缩放/查看细节，关闭窗口后本脚本结束）...')
    try:
        plt.show()
    except Exception as e:
        print(f'提示: 交互图形窗口未能打开({e})，所有图表已保存在 figures/ 目录中。')
else:
    print('\n[提示] 检测到非交互参数/环境变量，跳过弹窗，图表已全部保存在 figures/ 目录。')
