...
out_mask = (panel['year'] >= 2025) & ~((panel['year']==2026)&(panel['quarter']>=2))  # 09-09:恢复2025Q3(双源验证为真);2026Q2因子缺失,描述性保留、不入OOS
...
json_path = os.path.join(OUT, '样本外检验_2006_2024_2026Q1_5Q.json')
with open(json_path, 'w', encoding='utf-8') as f:
    json.dump(res_oos, f, ensure_ascii=False, indent=2)

csv_path = os.path.join(OUT, '样本外基金得分与表现明细_20260909_5Q.csv')
full_eval.reset_index().to_csv(csv_path, index=False, encoding='utf-8-sig')

# Also write to scripts/_oos_validation_20260904.py
this_code = open(__file__, encoding='utf-8').read()
open(os.path.join(BASE, 'scripts', '_oos_validation_20260909_5Q.py'), 'w', encoding='utf-8').write(this_code)

print(f'\n已成功输出结构化核算数据：\n  JSON -> {json_path}\n  CSV  -> {csv_path}\n  Script -> scripts/_oos_validation_20260904.py')
